...
sqlite-utils query $DB "INSERT INTO event SELECT et.*, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP, 0 FROM event e RIGHT JOIN event_temp et ON e.id=et.id WHERE e.id IS NULL OR (e.type != et.type OR e.sort_date != et.sort_date OR e.event != et.event OR e.fuzz != et.fuzz OR e.comment != et.comment OR e.source != et.source OR e.access_type != et.access_type OR e.onboard_type != et.onboard_type OR e.detailed_from != et.detailed_from OR e.detail_id != et.detail_id OR e.case_no != et.case_no) ON CONFLICT(id) DO UPDATE set date=excluded.date, sort_date=excluded.sort_date, event=excluded.event, fuzz=excluded.fuzz, comment=excluded.comment, source=excluded.source, access_type=excluded.access_type, onboard_type=excluded.onboard_type, detailed_from=excluded.detailed_from, detail_id=excluded.detail_id, case_no=excluded.case_no, created_at=excluded.created_at, modified_at=CURRENT_TIMESTAMP, mod_count=excluded.mod_count+1" > /dev/null
sqlite-utils drop-table event_temp --silent

            ...
            if event["type"] == "access_granted":
                for system_id in event.get("access_systems", []):
                    ...
                    if key in open_access:
                        raise ValueError(
                            f"Grant access {name} to {system_id} on {event['date']}, but has access"
                        )
                ...
                for system_id in event.get("access_systems", []):
                    ...
                    if key not in open_access:
                        raise ValueError(
                            f"Change access of {name} to {system_id} on {event['date']}, but doesn't have access"
                        )
                ...
                for system_id in event.get("access_systems", []):
                    ...
                    if key not in open_access:
                        print(closed_access)
                        raise ValueError(
                            f"Revoke access of {name} to {system_id} on {event['date']}, but doesn't have access"
                        )
                ...
                for system_record in open_records:
                    ...
                    print(
                        f"Auto-closing system access of {name} to system {system_id} on {event['date']}"
                    )
                    key = f"{name}|{system_id}"
                    system_record["end_date"] = event["date"]
                    closed_access.append(system_record)
...
meta = {"title": "Systems", "version": FILE_VERSION, "generated": date.today()}

systems = get_systems_postings_yaml()
dump_generated_file(
    meta,
    {"systems_access": systems},
    "system_postings.yaml",
    SCHEMA_PATH,
    line_break_indent=2,
)
